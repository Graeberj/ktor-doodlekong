...
import com.plcoding.util.getRandomWords
import com.plcoding.util.matchesWord
import com.plcoding.util.transformToUnderscores
import com.plcoding.util.words
...
class Room(
    ...
    private fun isCorrectlyGuessed(guess: ChatMessage): Boolean{
    ...
    private fun waitingForPlayers(){
        GlobalScope.launch {
            ...
            val phaseChange = PhaseChange(
                ...
                DELAY_WAITING_TO_START_TO_NEW_ROUND
            )
            broadcast(gson.toJson(phaseChange))
        }
    }
    ...
    private fun gameRunning(){
        ...
        GlobalScope.launch {
            ...
            println("Drawing phase in room $name started. It will last ${DELAY_GAME_RUNNING_TO_SHOW_WORD / 1000} seconds")
        }
    }
        ...
        if(isCorrectlyGuessed(message)){
            ...
            if(isRoundOver){
                val roundOverAnnouncement = Announcement(
                    "Everyone has guessed the word!! Time to start the new round",
                    System.currentTimeMillis(),
                    Announcement.TYPE_EVERYBODY_GUESSED_WORD
                )
    ...
    private suspend fun sendWordToPlayer(player: Player){
        val delay = when(phase){
            Phase.WAITING_TO_START -> DELAY_WAITING_TO_START_TO_NEW_ROUND
            Phase.NEW_ROUND -> DELAY_NEW_ROUND_TO_GAME_RUNNING
            Phase.GAME_RUNNING -> DELAY_GAME_RUNNING_TO_SHOW_WORD
            Phase.SHOW_WORD -> DELAY_WAITING_TO_START_TO_NEW_ROUND
            else -> 0L
        }
    ...
    companion object {
        ...
        const val DELAY_WAITING_TO_START_TO_NEW_ROUND = 10000L
